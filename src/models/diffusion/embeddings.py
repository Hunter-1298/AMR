...
class TimestepEmbedding(nn.Module):
    def __init__(
        ...
        time_embed_dim: int,
        act_fn: str = "silu",
        out_dim: int = None,  # pyright: ignore
        post_act_fn: Optional[str] = None,  # pyright: ignore
        cond_proj_dim=None,
    ...
    def forward(self, sample, condition=None):
        if condition is not None:
            sample = sample + self.cond_proj(condition)  # pyright: ignore
...
class LabelEmbedding(nn.Module):
    ...
    def forward(self, labels: torch.LongTensor, force_drop_ids=None):
        ...
        if (self.training and use_dropout) or (force_drop_ids is not None):
            labels = self.token_drop(labels, force_drop_ids)  # pyright: ignore
