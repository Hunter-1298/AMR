...
class DiffusionVisualizationCallback(L.Callback):
    ...
    def create_diffusion_visualizations(self, model, batch):
        ...
        with torch.no_grad():
            for i, t in enumerate(timesteps):
                ...
                axes_top_freq[i].set_title(
                    f"t={t_val:.0f}\nFreq. domain SNR est: {freq_domain_snr:.1f} dB"
                )
        ...
        row2_pos = (row1_bottom + row2_top - 0.025) / 2  # Between row 1 and 2
        row3_pos = (row2_bottom + row3_top - 0.025) / 2  # Between row 2 and 3
        row4_pos = (row3_bottom + row4_top - 0.025) / 2  # Between row 3 and 4
